...
const temperatureData = computed(() =>
    analytics.value.slice(0, 7).map((item, i) => ({
...
const temperaturePoints = computed(() => {
  ...
  return data.map((p, i) => `${i * 50 + 25},${180 - (p.value / maxTemp) * 150}`).join(' ');
});

const temperatureAreaPoints = computed(() => {
  const pts = temperaturePoints.value;
  if (pts === '25,90') return '25,180 25,180';
  return `25,180 ${pts} ${(temperatureData.value.length - 1) * 50 + 25},180`;
});
...
const humidityAreaPoints = computed(() => {
  const pts = humidityPoints.value;
  if (pts === '25,90') return '25,180 25,180';
  return `25,180 ${pts} ${(humidityData.value.length - 1) * 50 + 25},180`;
});
...
const loadData = async () => {
  ...
  try {
    ...
    analytics.value = plants.value
        ...
        .map(plant => analyticsService.calculateAnalyticsFromMetrics(
            plant.id,
            plant.metrics,
            plant.metrics[0]?.deviceId
        ));

    if (analytics.value.length) {
      const n      = analytics.value.length;
      ...
      summary.value = {
        avgTemperature:  Math.round(totals.avgTemperature  / n),
        avgHumidity:     Math.round(totals.avgHumidity     / n),
        avgSoilMoisture: Math.round(totals.avgSoilMoisture / n),
        avgLight:        Math.round(totals.avgLight        / n),
...
const loadHistoricalData = async () => {
  ...
  try {
    ...
    // histórico es opcional, no se muestra error
  } finally {
    loadingHistory.value = false;
  }
};

onMounted(() => Promise.all([loadData(), loadHistoricalData()]));
</script>
